...
const WalkInForm = ({
  ...
  // ✅ AUTO-CALCULATE AMOUNT from selected services
  useEffect(() => {
    if (selectedServices.length > 0) {
      const total = selectedServices.reduce((sum, s) => sum + s.price, 0);
      // Create synthetic event to trigger onInputChange
      onInputChange({
        target: {
          name: 'amount',
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="p-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                ...
                {/* ✅ ADDED: Amount field (auto-calculated but editable) */}
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">
                    Amount *
                  </label>
                  <input
                    ...
                    required
                  />
                  <p className="text-xs text-gray-500 mt-1">Auto-calculated from services (editable)</p>
                </div>
